function results = run_KCF_AST(seq, res_path, bSaveImage)
	...
	feature_type = 'gray';
	show_visualization =0;% ~strcmp(video, 'all');
	show_plots =0;% ~strcmp(video, 'all');
    ...
    num_init = 3;  %number of samples for initializing the Gaussian model
...
% 		seq = evalin('base', 'subS');
		...
		[positions,fps] = OCT_KCF(video_path, img_files, pos, target_sz, ...
