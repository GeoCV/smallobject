...
% 	window_sz = 2 .^ nextpow2(window_sz);
    ...
    for m=1:window_sz(1)
        for n=1:window_sz(2)%-((m-floor(window_sz(1)/2)^2+((n-floor(window_sz(2)/2)))^2)/5000)
            guassian(m,n)=1;%exp(-(0.000000001*n*(m-floor(window_sz(1)/2))^2+0.000000001*m*(n-floor(window_sz(2)/2))^2));
        end
    end
...
% 		end
		...
		if frame > 1,
			...
			patch = get_subwindow(im, pos, window_sz);%patch(:,:,1)=patch(:,:,1).*guassian;patch(:,:,2)=patch(:,:,2).*guassian;patch(:,:,3)=patch(:,:,3).*guassian;
            ...
            if frame >= num_init
                ...
                if (max_res(frame) < 0.25 && (temp_init > 0.35))||...
                    ...
                    coarse_regions = qdct_coarse_sampler(pos,window_sz,im);                    
		...
		patch = get_subwindow(im, pos, window_sz);
        patch(:,:,1)=patch(:,:,1).*guassian;%patch(:,:,2)=patch(:,:,2).*guassian;patch(:,:,3)=patch(:,:,3).*guassian;
